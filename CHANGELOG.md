...
- Bump the `TestShardExactRouting_FullPipeline_Integration` poll budget from 4 to 32 attempts as a temporary stopgap for a freshly created index's `routing_num_shards` not yet propagating from `/_cluster/state/metadata` under CI CPU starvation. The proper fix is a condition-based wait on metadata propagation ([#957](https://github.com/opensearch-project/opensearch-go/issues/957))
...
- Fix the single-node connection pool bypassing the seed-URL fallback when node discovery replaces the seed with one unroutable node. PR #952 gave the multi-server pool an availability gate, but when discovery yields exactly one node the transport builds a `singleServerPool`, whose `Next()` returned its sole connection unconditionally. A discovered node is minted `lcDead|lcNeedsWarmup|lcNeedsHardware` and, when its `publish_address` is unroutable from the client (NAT'd or misconfigured clusters, e.g. a Kubernetes stack cluster in CI), the request dialed the pod IP and failed with `no route to host` -- a transport error, not `ErrNoConnections`, so the post-loop seed fallback (which triggers only on `ErrNoConnections`) never fired and every request died on the unreachable node. `singleServerPool.Next()` now applies the same `availableForRouting()` gate: a never-verified discovered node (still carrying `lcNeedsHardware`) makes the pool report `ErrNoConnections`, so the request cascades to the seed fallback until the node health-checks clean. A user-supplied seed connection is always available (`availableForRouting` short-circuits on `c.seed`), so a genuine single-seed-node pool is unaffected ([#956](https://github.com/opensearch-project/opensearch-go/pull/956))
- Fix node discovery hijacking the request stream with unverified, unreachable discovered nodes and masking the seed-URL fallback. Discovery inserts freshly discovered connections into a policy pool's dead list before they are health-checked (`lcDead|lcNeedsWarmup|lcNeedsHardware`), and each policy cached its enabled bit as `len(ready)+len(dead) > 0` -- so a policy holding only never-verified discovered connections still reported enabled. `Route` then handed the request to the pool, which served the unverified connection as a zombie instead of returning `ErrNoConnections`, so the request never cascaded to the user-supplied seed-URL fallback (which triggers only on `ErrNoConnections`). When discovered `publish_address` values are unroutable from the client (NAT'd or misconfigured clusters, e.g. a Kubernetes stack cluster in CI), this failed every request with `no route to host` even though the seed URL was reachable. A connection is now considered available for routing only if it is a user-supplied seed (assumed reachable) or a discovered connection confirmed reachable (`lcNeedsHardware` cleared); the round-robin, role, coordinator, and index/doc-router policies compute their enabled bit from availability rather than presence, so an all-unverified pool reports not-enabled and the seed fallback serves requests until a discovered node health-checks clean ([#952](https://github.com/opensearch-project/opensearch-go/pull/952))
